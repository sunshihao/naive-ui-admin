...
function hback(){
    ...
    //forbid SHIFT+CLICK
    else if ((event.srcElement.tagName=="A") && (event.shiftKey))
    {
    	retVal = false;
    }
    //forbid F5 And F11
    else if (event.keyCode==116 || event.keyCode==122)
    {
         event.keyCode=0;
         retVal = false;
    }
    //forbid BackSpace
         ...
         if (event.srcElement.type!=undefined && (event.srcElement.type=="text" || event.srcElement.type=="password" || event.srcElement.type=="file" || event.srcElement.type == "textarea") && (!event.srcElement.readOnly) && (!event.srcElement.disabled)){
        	 retVal = true;
         } else {
        	 retVal = false;
         }
 ...
 * json对象转为string
 * **/
function JsonToString(resultObj) {
