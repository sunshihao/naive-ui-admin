...
Class.pt.vessel = function(conType, callback){
    ...
    callback([
        ...
        //主体
        '<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') + '" invokedFunName="'+ config.invokedFunName + '" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
...
Class.pt.creat = function(){
    ...
    returnFunTemp = config.callBackFun;
...
Class.pt.callback = function(){
    ...
    if(config.success){
        if(config.type == 2){
        	var templ = layero.find('iframe')[0];
        	if(templ.attachEvent){//iLead 解决ie8下不触发onload的问题
        		templ.attachEvent("onload",function(){
                    this.className = '';
    ...
    //取消
    function cancel(){
        var close = config.cancel && config.cancel(that.layero, that.index);//iLead 多传一个layer对象参数
    ...
    //config.callBackFun && (ready.callBackFun[that.index] = config.callBackFun);
};
...
layer.callBackFun = function(obj, windowName) {
//	alert(currentIndex);
	returnFunTemp(obj);
//	var layero = $('#'+ doms[0] + currentIndex);
//	var iframeWin = window[layero.find('iframe')[0]['name']]; 
//	iframeWin.demo.UI4InterNotifEdit.doTest();
	//iframeWin.callBackFun(obj);
	//callBackFun(obj);
	//return userData;
};
...
layer.invokFun = function(windowName, param) {
    var index = $('#'+ windowName).parents('.'+doms[4]).attr('times');	
	var all = $(".layui-layer");
	var iframeWin;
	var curIndex;
	var fn;
	if(all.length > 1) {
		$.each(all, function(i, value){
			if(i + 2 == all.length) {
				if(parseInt($(value).attr("times")) < index) {
					//alert(i + "==" + $(value).attr("times"));
					curIndex = $(value).attr("times");
					var layero = $('#'+ doms[0] + curIndex);
					//iframeWin = window[layero.find('iframe')[0]['name']];
					iframeWin = "window[$('#" + doms[0] + curIndex + "').find('iframe')[0]['name']]";
					//iframeWin = "$(window.parent.document).contents().find(\"#layui-layer-iframe" + curIndex  + "\")[0].contentWindow";
					fn = $('#'+ windowName).parents('.'+doms[4]).attr("invokedfunname");
					return false;
				}				
			}
		...
		//alert("11");
		fn = $(".layui-layer").attr("invokedfunname");
		iframeWin = "$(window.parent.document).contents().find('iframe')[0].contentWindow";
	}
	var arr = new Array();
	if(param != null) {
		arr.push(param);
	}		
	eval(iframeWin + "." + fn).apply(this, arr);  
};

layer.getParentFrame = function(windowName) {
	var index = $('#'+ windowName).parents('.'+doms[4]).attr('times');
...
//增加获取用户数据方法  by zrzdemonz
layer.getUserData = function(){
	//alert(userData);
	return userData;
};

layer.setUserData = function(jsonObj){
