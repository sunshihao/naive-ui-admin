...
;(function ($) {
	...
	Plugin.prototype = {
		...
		setSelectedIcon: function (theIcon) {
			...
			//add by zrzdemon
			//this.iconPicker.find("option[text='pxx']").attr("selected",true);
			this.element.html("<option selected='selected' value='" + theIcon + "'></option>");
			//end
	...
	$.fn.fontIconPicker = function (options) {
		...
		this.setSelectIcon =  $.proxy(function (theIcon) {
			if (theIcon === 'fip-icon-block') {
				theIcon = '';
			}
			this.each(function () {
				// Check if attribute is to be used
				if ( $.data(this, "fontIconPicker").settings.useAttribute ) {
					if ( theIcon ) {
						$.data(this, "fontIconPicker").iconPicker.find('.selected-icon').html('<i ' + $.data(this, "fontIconPicker").settings.attributeName + '="' + ( $.data(this, "fontIconPicker").settings.convertToHex ? '&#x' + parseInt(theIcon, 10).toString(16) + ';' : theIcon ) + '"></i>' );
					} else {
						$.data(this, "fontIconPicker").iconPicker.find('.selected-icon').html('<i class="fip-icon-block"></i>');
					}
				// Use class
				} else {
					$.data(this, "fontIconPicker").iconPicker.find('.selected-icon').html('<i class="' + (theIcon || 'fip-icon-block') + '"></i>');
				}
				// Set the value of the element and trigger change event
				$.data(this, "fontIconPicker").element.val((theIcon === '' ? $.data(this, "fontIconPicker").settings.emptyIconValue : theIcon )).trigger('change');
				if ( $.data(this, "fontIconPicker").triggerEvent !== null ) {
					// Trigger other events
					for ( var eventKey in $.data(this, "fontIconPicker").triggerEvent ) {
						$.data(this, "fontIconPicker").element.trigger($.data(this, "fontIconPicker").triggerEvent[eventKey]);
					}
				}
				$.data(this, "fontIconPicker").currentIcon = theIcon;
				$.data(this, "fontIconPicker").setHighlightedIcon();
			});
			
		}, this);

		this.getSelectIcon =  $.proxy(function () {
			var result;
			this.each(function () {
				result = $.data(this, "fontIconPicker").currentIcon;
			});
			return result;
		}, this);
