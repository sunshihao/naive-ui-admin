...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			//16/11/24 jinmingjie
			if(len == 0){
				if($(ts).next(".ui-jqgrid-emptyMsg").length == 0){
					$(ts).after("<div class='ui-jqgrid-emptyMsg'>"+ts.p.emptyMsg+"</div>");
				}
			}else{
				$(ts).next(".ui-jqgrid-emptyMsg").remove();
			}
			while (i<len) {
				cur = drows[i];
				idr = $.jgrid.getAccessor(cur,idn);
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			//$(ts).jqGrid().dataMap.clear();
			$(ts).jqGrid("getGridParam", "dataMap").clear();
...
$.jgrid.extend({
	...
	navGrid : function (elem, p, pEdit, pAdd, pDel, pSearch, pView, pExportExcel) {
		...
		p = $.extend({
			...
			//add by zrzdemon export execl start
			exportExcel:true,
			exportExcelicon:"glyphicon glyphicon-export",
			//export execl end
			view: false,
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				pExportExcel = pExportExcel || {};
				if(o.exportExcel) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='"+o.exportExcelicon+"'></span>"+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl).attr({"title":"导出表格"  || "",id: "exportExcel"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							//alert("ok");
							$($t).jqGrid("exportExcelAction");
						}
						return false;
					}).hover(onHoverIn, onHoverOut);
					tbd = null;
...
//					alert($t.p.pager);
//					$($t).jqGrid('navGrid',$t.p.pager,{}).jqGrid('navButtonAdd', $t.p.pager,{caption:"Add",
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							 ...
							 * 加入bootstrap样式
							 * **/
							$(elc).addClass("editable form-control");
							/**
							 * 加入jquery validate方式
							 * **/
							if(typeof(cm[i].jqvalid) != "undefined" && cm[i].jqvalid != null) {
								
								$(elc).addClass(JSON.stringify(cm[i].jqvalid).replaceAll("\"",""));
							}
							
							var sourceValue = $(elc).val();
						
							$(elc).focusout(function(){
								//alert($(this).val());
								var dataMap = $($t).jqGrid("getGridParam", "dataMap");
								var obj = null;
								if(dataMap != null) {
									//alert($(this).val() + "====" + sourceValue);
									if($(this).val() != sourceValue) {
										
										//dataMap.get(rowid).status = "1";
										obj = dataMap.get(rowid);
										if(typeof(obj) != "undefined" && obj != null) {
											obj.status = "2";
											$($t).jqGrid("getGridParam", "dataMap").put(rowid, obj);
										}
									}else if(typeof(cm[i].formatter) != "undefined" && typeof cm[i].formatter == "string" && cm[i].formatter.toLowerCase() == "date") {
										obj = dataMap.get(rowid);//typeof cm[i].formatter == "string" add by Alex
										if(typeof(obj) != "undefined" && obj != null) {
											obj.status = "2";
											$($t).jqGrid("getGridParam", "dataMap").put(rowid, obj);
										}
					...
					if(o.keys===true) {
						$(ind).bind("keydown",function(e) {
							...
							if (e.keyCode === 13) {
								...
								// update by zrzdemon
								if( $($t).jqGrid("saveRowNew", rowid, o ) ) {
									if($t.p.inlineNav) {
										try {
											$($t).jqGrid('showAddEditButtons');
										} catch (eer2) {}
